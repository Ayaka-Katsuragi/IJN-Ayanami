...
bot.on("ready", async () => {
	...
	bot.user.setActivity("Android Sys Detection API|No Game Detected!", {type: "PLAYING"});
...
bot.on("message", async message => {
	...
	if(cmd === `${prefix}device-detect`){
		if(!message.member.hasPermission("MANAGE_MESSAGES")) return message.channel.send("Invalid User Permissions!!");
		return message.channel.send("📱 Samsung Galaxy J3 Luna Detected 📱. Mobile game states will be traced and displayed on my status ⚠");
	}
